...
public class Utils {
	...
	public static String checkHasParas(String url, Map<String, Object> map, String encoding) throws Exception {
		if (map == null) {
			map = new HashMap<>();
		}
		List<NameValuePair> nvps = new ArrayList<>();
		if (url.contains("?") && url.indexOf("?") < url.indexOf("=")) {
			map = buildParas(url.substring(url
					.indexOf("?") + 1), map);
		}
		if (url.contains("?")) {
			url = url.substring(0, url.indexOf("?"));
...
//							builder.addPart(String.valueOf(map.get(ENTITY_MULTIPART+".name")) + count++, fileBody);
	 ...
	 * @return
	 */
	public static Map<String, Object> buildParas(String paras, Map<String, Object> map) {
		String[] p = paras.split("&");
		String[][] ps = new String[p.length][2];
		int pos = 0;
		for (int i = 0; i < p.length; i++) {
